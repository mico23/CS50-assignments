...
//no need to use break, if the previous statement is return.
...
int duration(string fraction)
{
    // TODO
    //char converter = ;
    //int denominator = atoi(converter); //check how to use atoi; seems char wont work
    int numerator;
    ...
    switch(fraction[2])
    {
        ...
        break; //no need a break if the previous statement is a return
    ...
    return numerator;
    //if the return is nest inside the switch function
    //this function will not work (?);
    //it won't return the expected value.
}
...
int frequency(string note)
{
    ...
    {
        if (note[0] == 'C') //should be ""(string) instead of ''(char)
        {
            count_note = 9;
